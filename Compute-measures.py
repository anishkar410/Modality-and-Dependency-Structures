...
for i in ud_files:                                       # reads file of each language one by one
    ...
    dfile = open(str(i),'r',encoding='utf-8')
            ...
            if n<12 and n>1:
                ...
                for edgey in tree.edges:
                    if not edgey[0]==0:
                        ...
                        dep_heads_of_intervener_types_real=get.heads_of_intervener_types(edgey)
                        
                        #print(dep_distance_real)
                        results2 = open('English-measures.csv','a')
                        results2.write(str(lang)+"\t"+"real"+"\t"+str(sent_id)+"\t"+str(n)+"\t"+str(projection_degree_real)+"\t"+str(edgey)+"\t"+str(direction_real)+"\t"+str(dep_distance_real)+"\t"+str(dep_depth_real)+"\t"+str(dep_intervener_real)+"\t"+str(dep_heads_of_intervener_types_real)+"\n")
                        results2.close()
                    print("\n-----------------\n"+str(tree.edges))
