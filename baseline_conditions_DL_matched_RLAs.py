...
##                    yield frozenset({edge, edge_})
    ...
    def rand_tree(self,num_cross_real):                 # requires number of crossings from the real tree
        ...
        for nodeI in node_list:
            treex.add_node(nodeI,form=self.tree.nodes[nodeI]['form'],upostag=self.tree.nodes[nodeI]['upostag'],feats=self.tree.nodes[nodeI]['feats'],deprel=self.tree.nodes[nodeI]['deprel'])
