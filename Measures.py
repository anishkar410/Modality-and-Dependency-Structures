...
class Compute_measures(object):
    ...
    def heads_of_intervener_types(self, edge):
        nodex_between = []
        heads_of_nodex = []
        types = []
        rel = []
        ...
        if edge[0] > edge[1]:
            for nodex in nx.descendants(self.tree, self.root):
                if edge[1] < nodex < edge[0]:
                    pos_nodex = self.tree.nodes[nodex].get('upostag')
                    if pos_nodex is not None:
                        nodex_between.append(nodex)
        else:
            for nodex in nx.descendants(self.tree, self.root):
                if edge[0] < nodex < edge[1]:
                    nodex_between.append(nodex)
        
        for nodex in nodex_between:
            for in_edge in self.tree.in_edges(nodex):
                head_node = in_edge[0]
                head_pos = self.tree.nodes[head_node].get('upostag')
                pos_nodex = self.tree.nodes[nodex].get('upostag')
                if self.tree.has_edge(head_node, nodex):
                    int_rel = self.tree[head_node][nodex].get('drel')
                    if pos_nodex is not None:
                        types.append(pos_nodex)
                        rel.append(int_rel)
                    heads_of_nodex.append((pos_nodex, head_pos, int_rel))

        return heads_of_nodex

    def heads_types(self, edge):
        nodex_between = []
        heads_of_nodex = []
        ...
        for nodex in nodex_between:
            for in_edge in self.tree.in_edges(nodex):
                ...
                if head_pos is not None:
                    heads_of_nodex.append(head_pos)
        
        return heads_of_nodex
    
    

    def intervener_pos(self, edge):
        ...
        if edge[0] > edge[1]:
            for nodex in nx.descendants(self.tree, self.root):
                if edge[1] < nodex < edge[0]:
                    pos_nodex = self.tree.nodes[nodex].get('upostag')  
                    print(f"Found pos_nodex: {pos_nodex} for node {nodex}")  
                    if pos_nodex is not None:
                        types.append(pos_nodex)
        else:
            for nodex in nx.descendants(self.tree, self.root):
                if edge[0] < nodex < edge[1]:
                    ...
                    if pos_nodex is not None:
                        types.append(pos_nodex)
        print(f"Types for edge {edge}: {types}")  
        return types
    ...
    def relation_pos(self, edge):
        print(f"Processing edge: {edge}")  
        kind_of = []

        pos_head = self.tree.nodes[edge[0]].get('upostag')  
        print(f"Found pos_head: {pos_head} for head node {edge[0]}")  
        if pos_head is not None:
            kind_of.append(pos_head)

        pos_dependent = self.tree.nodes[edge[1]].get('upostag')  
        print(f"Found pos_dependent: {pos_dependent} for dependent node {edge[1]}") 
        if pos_dependent is not None:
            kind_of.append(pos_dependent)

        print(f"Types for edge {edge}: {kind_of}") 
        return kind_of
